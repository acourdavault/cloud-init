...
from cloudinit.sources import DataSourceLinode
from cloudinit.sources.helpers import linode

from cloudinit.tests.helpers import mock, CiTestCase

# Linode metadata test data
VULTR_V1_1 = {
    'bgp': {
        'ipv4': {
            'my-address': '',
...
class TestDataSourceLinode(CiTestCase):
    def setUp(self):
        super(TestDataSourceLinode, self).setUp()
    ...
    @mock.patch('cloudinit.sources.helpers.linode.is_linode')
    @mock.patch('cloudinit.sources.helpers.linode.get_metadata')
    def test_datasource(self,
                        mock_getmeta,
                        mock_islinode,
                        mock_netmap):
        mock_getmeta.return_value = VULTR_V1_2
        mock_islinode.return_value = True
        mock_netmap.return_value = INTERFACE_MAP

        source = DataSourceLinode.DataSourceLinode(
    ...
    def test_network_config(self, mock_netmap):
        ...
        self.assertEqual(EXPECTED_VULTR_NETWORK_1,
                         linode.generate_network_config(interf))
